...
handlers += [
    ...
    ConversationHandler(
        entry_points=[CommandHandler('shutdownserver',
                                     server_callbacks.shutdown_declaration)],
        states={
    ...
    ConversationHandler(
        entry_points=[CommandHandler('rebootserver',
                                     server_callbacks.reboot_declaration)],
        states={
    ...
    ConversationHandler(
        entry_points=[CommandHandler(
            'runtests', server_callbacks.manually_run_tests_declaration
        )],
        states={
            1: [MessageHandler(Filters.text,
                               server_callbacks.manually_run_tests)]
        },
        fallbacks=[CommandHandler('cancel', callbacks.cancel)]
    ),
    ConversationHandler(
        entry_points=[CommandHandler('sendnews',
                                     server_callbacks.sendnews_declaration)],
        states={
            'get_message': [MessageHandler(Filters.text,
                                           server_callbacks.get_message)],
            'confirm_message': [MessageHandler(
                Filters.text,
                server_callbacks.confirm_message
            )],
            'distribute_message': [CallbackQueryHandler(
                server_callbacks.distribute_message)],
            },
        fallbacks=[CommandHandler('cancel', callbacks.cancel)]
    ),
]
...
handlers += [
    ...
    CommandHandler('help', callbacks.help),
...
# General Handlers
handlers += [
    MessageHandler(Filters.command, callbacks.unknown_handler),
    MessageHandler(Filters.text, callbacks.unknown_handler),
]
...
TEST_INTERVAL = 3600 * 24 * 2 # Every two days otherwise gogoanime links expire
job_queue = updater.job_queue
job_queue.run_repeating(server_callbacks.auto_run_tests,
                        interval=TEST_INTERVAL, first=0)
job_queue.run_repeating(callbacks.auto_update_users,
...
if __name__ == '__main__':
    ...
    if ARGS.method == 'poll':
        ...
        updater.start_webhook(listen='0.0.0.0',
                              ...
                              cert='cert.pem',
                              webhook_url=(
                                  "https://www.fitiwall.com:{}/{}"
                                  .format(Credentials.ACCESS_PORT,
